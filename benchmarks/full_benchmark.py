...
def main():
    ...
    print("=" * 70)
    print(f" Device: {torch.cuda.get_device_name() if device.type == 'cuda' else device.type.upper()}")
    ...
    all_results["preprocessing"] = preprocess_results
    ...
    if preprocess_results["gpu"]:
        ...
        best_size = max(preprocess_results["cpu"].keys(),
                       key=lambda s: preprocess_results["cpu"][s] / preprocess_results["gpu"].get(s, 1))
        best_speedup = preprocess_results["cpu"][best_size] / preprocess_results["gpu"][best_size]

        if device.type == "cuda":
            print(f" Preprocessing: GPU Decode (NVJPEG) avg {avg_preprocess_speedup:.1f}x faster")
            print(f"                Best: {best_size} -> {best_speedup:.1f}x faster")
        else:
    ...
    # Inference summary
    if not args.skip_inference and inference_results:
        print(f" Inference:")
        for model, batches in inference_results.items():
            batch_1 = batches.get("batch_1", {})
            if batch_1:
